...
def compute_velocity_fields(F, exp_type, name, color='k'):
    ...
    if exp_type == "div":
        ...
        u, v, F_recomp, u_noise, v_noise = synthetic_deformations(F, name, color)
...
def synthetic_deformations(F, name, color, dx=1, dy=1, vel_fig=False, shear_fig=False):
# SEPARATED
    """
    ...
    # Define the sparse finite differences matrices (2N, 2N)
    zero_matrix = lil_matrix((N2, N2))
    A_sparse = create_sparse_matrix_dx(N).tolil()
    B_sparse = create_sparse_matrix_dy(N).tolil()
    D_sparse = create_sparse_matrix_dy_shear(N).tolil()
    C_sparse = create_sparse_matrix_dx(N).tolil()  # for div, wrong !!
    #AB_sparse = bmat([[A_sparse, zero_matrix], [zero_matrix, B_sparse]], format="lil")
    AB_sparse = bmat([[A_sparse, zero_matrix, zero_matrix, zero_matrix], [zero_matrix, B_sparse, zero_matrix, zero_matrix], [zero_matrix, zero_matrix, C_sparse, zero_matrix], [zero_matrix, zero_matrix, zero_matrix, D_sparse]], format="lil")
    AB_sparse = AB_sparse.tocsr()
